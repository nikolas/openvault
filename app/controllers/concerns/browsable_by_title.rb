module BrowsableByTitle
  
  def browse_by_title
    model = self.class.to_s.sub('Controller','')
    model = 'Program' if model == 'Programs' # TODO: More consistent class naming would simplify this.
    @results = Blacklight.solr.select(params: {q: "has_model_ssim:info:fedora/afmodel:#{model}", rows: 10000})['response']['docs']

...
describe Openvault::Pbcore do
  ...
  describe '.get_model_for' do
    ...
    it 'returns a new, unsaved model if values from PbcoreDescDoc#all_ids have not yet been saved' do
      ...
      Openvault::Pbcore.get_model_for(pbcore_desc_doc).persisted?.should == false
    end


    it 'checks to see if model exists with values for <pbcoreIdentifier> and if so, returns that model' do
      model = Openvault::Pbcore.get_model_for(Fixtures.use('artesia/rock_and_roll/series_1.xml'))
      model.save!
      Openvault::Pbcore.get_model_for(Fixtures.use('artesia/rock_and_roll/series_1.xml')).should == model
    end
  ...
  describe '.ingest!' do
    ...
    context 'with a subset of related assets that have been transformed from Artesia xml' do
      before :all do
        ...
        Openvault::Pbcore.ingest! Fixtures.raw('artesia/rock_and_roll/related_assets_subset.xml')

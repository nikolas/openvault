...
class more_types_object extends more_plugins_object_sputnik_7 {
	...
	function add_boxes_to_post_type () {
		...
		foreach ($pages as $name => $page) {
			foreach((array) $page['boxes'] as $box_key) {
				...
				$position = ($box['position'] == 'left') ? 'normal' : 'advanced';
...
/*
			...
			*/
			...
			// Set if not set
		 	if (!$options['query_var']) $options['query_var'] = true;
			if (!$options['menu_position']) $options['menu_position'] = null;
